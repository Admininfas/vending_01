...
Este modelo persiste todos los requests recibidos en los endpoints
de webhooks para auditoria y debugging.
"""
...
class VendingWebhookLog(models.Model):
    ...
    endpoint = fields.Selection([
        ...
        ('load', 'Load Webhook'),
    ], string='Endpoint', required=True, help='Tipo de webhook recibido')
    ...
    remote_addr = fields.Char(
        ...
        size=45,
        help='Dirección IP del cliente que envió el request'
    )
    ...
    @api.depends('endpoint', 'create_date', 'status_code')
    def _compute_display_name(self):
        ...
        for record in self:
            ...
            record.display_name = f"{endpoint_name} - {date_str} ({record.status_code or 0})"
